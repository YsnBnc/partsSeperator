...
There is a ```ITERATION_THRESHOLD``` limit for the servomotor to activate. The reason for it model spams output if something detected or not detected. Problem with it is it can detect randomly and send false signal to motor. The ```ITERATION_THRESHOLD``` prevents that happening like if model gives 10 consecutive outputs then servomotor turns.
